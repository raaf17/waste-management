...
const sidebarItems = [
    ...
    { href: '/report', label: 'Reports Waste', icon: MapPin },
            ...
            <nav className='h-full flex flex-col justify-between'>
                <div className='px-4 py-6 space-x-8'>
                    {sidebarItems.map((item) => (
                        <Link href={item.href} key={item.href} prefetch={false}>
                            <Button variant={pathname === item.href ? 'secondary' : 'ghost'} className={`w-full justify-start py-6 mb-1 ${pathname === item.href ? 'bg-green-100 text-green-800' : 'text-gray-800 hover:bg-gray-100'}`}>
                ...
                <div className='p-4 border-t border-gray-200'>
                    <Link href="/settings" passHref>
                        <Button variant={pathname === '/settings' ? 'secondary' : 'outline'} className={`w-full py-5 ${pathname === '/settings' ? 'bg-green-100 text-green-800' : 'text-gray-600 border-gray-300 hover:bg-gray-100'}`}>
